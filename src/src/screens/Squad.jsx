...
const calcOverall = (p) => {
  ...
  const w = weights[p.pos] || weights.MC;
  const stats = { rhythm: p.rhythm, shooting: p.shooting, passing: p.passing, defense: p.defense, dribbling: p.dribbling, physical: p.physical };
  return Math.round(Object.keys(w).reduce((sum, key) => sum + (stats[key] || 0) * w[key], 0));
};
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              ...
              <span
                style={{
                  fontSize: 11,
                  background: posColor[selected.pos],
                  color: "#fff",
                  padding: "2px 8px",
                  borderRadius: 4,
                }}
